    ...
    def set_compression(self, compression: str) -> None:
        ...
        try:
            ...
            self.cancel_by_local(
                TriRpcStatus(GRpcCode.INTERNAL, description="Unsupported compression")
            )
...
class ServerTransportListener(Http2Stream.Listener):
    ...
    def on_headers(self, headers: Http2Headers, end_stream: bool) -> None:
        ...
        if len(parts) != 3:
            self._response_error(
                TriRpcStatus(
                    GRpcCode.UNIMPLEMENTED, description=f"Bad path format: {path}"
                )
            )
            return
        ...
        self._decoder = TriDecoder(
            ServerTransportListener.ServerDecoderListener(self._listener), decompressor
        )
    ...
    def _response_plain_text_error(self, code: int, status: TriRpcStatus) -> None:
        ...
        headers.add(
            TripleHeaderName.CONTENT_TYPE.value, TripleHeaderValue.TEXT_PLAIN_UTF8.value
        )
